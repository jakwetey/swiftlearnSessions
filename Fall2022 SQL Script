...
----09/20/2022
--VIEW -- ITS A VIRTUAL TABLE BASED ON THE RESULT SET OF A SQL STATEMENT

--TYPES
---MATERIALIZED  -- IS UPDATED WHEN UNDERLYING TABLE(S) IS UPDATED.
---NON MATERIALIZED--- IS NOT UPDATED WHEN UNDERLYING TABLE(S) IS UPDATED.
...
SELECT PRODUCTNAME, UNITPRICE, CATEGORYID
FROM VW_CAT_UNITPRICE 
WHERE RANKING IN(1,2);

CREATE VIEW VW_PRODUCTION_PRODUCTS
AS
SELECT * FROM PRODUCTION.PRODUCTS;
SELECT * FROM VW_PRODUCTION_PRODUCTS;

--INLINE TABLE-VALUED FUNCTION
CREATE FUNCTION FN_CAT_UNITPRICE(@RANK1 INT, @RANK2 INT)
RETURNS TABLE
AS
	RETURN
	SELECT * FROM (SELECT PRODUCTNAME, UNITPRICE, CATEGORYID,
					 ROW_NUMBER() OVER(PARTITION BY CATEGORYID ORDER BY UNITPRICE)AS RANKING
				   FROM PRODUCTION.PRODUCTS) AS T
	WHERE RANKING IN (@RANK1 , @RANK2)

----SELECT FROM FUNCTION
SELECT PRODUCTNAME, UNITPRICE, CATEGORYID 
FROM FN_CAT_UNITPRICE(2,3);

SELECT PRODUCTID, NAME, PRODUCTNUMBER,(LISTPRICE -STANDARDCOST) PROFIT
FROM PRODUCTION.PRODUCT
ORDER BY LISTPRICE DESC

--- USER-DEFINED SCALAR FUNCTION
CREATE FUNCTION  FN_PROFITS(@PRICE MONEY, @COST MONEY)
RETURNS MONEY
	BEGIN
		RETURN (@PRICE - @COST)
	END

SELECT dbo.FN_PROFITS (200, 52)

SELECT PRODUCTID, NAME, PRODUCTNUMBER,LISTPRICE,STANDARDCOST, 
	   dbo.FN_PROFITS (LISTPRICE, STANDARDCOST)
FROM PRODUCTION.PRODUCT

--DROP FUNCTION dbo.FN_PROFITS;

SELECT ORDERID, PRODUCTID, UNITPRICE, DISCOUNT,
		 ADVENTUREWORKS.DBO.FN_PROFITS (UNITPRICE, DISCOUNT)PRICE
FROM TSQL2012.SALES.ORDERDETAILS

--STORED PROCEDURE
---- ITS A PRECOMPILED SQL STATEMENT THAT IS RUN REPEATEDLY.
----SYNTAX
--CREATE PROCEDURE SPNAME (@PARAMETER DATATYPE)
--AS
--	BEGIN
--		SQL STATEMENT
--	END;
...
---MODIFYING PROCEDURE TO ACCEPT INPUT VALUES
CREATE OR ALTER PROCEDURE SP_PRODUCTS (@LISTPRICE MONEY)
AS
	BEGIN
		SELECT NAME, LISTPRICE
		FROM PRODUCTION.PRODUCT
...
--CALLING PROCEDURE
EXECUTE SP_PRODUCTS 1650.65

-- MODIFYING PROCEDURE  TO ACCEPT MULTIPLE INPUT VALUES
---MODIFYING PROCEDURE TO ACCEPT INPUT VALUES
CREATE OR ALTER PROCEDURE SP_PRODUCTS (@MINPRICE MONEY, @MAXPRICE MONEY )
AS
	BEGIN
		SELECT NAME, LISTPRICE
		FROM PRODUCTION.PRODUCT
...
---IF ELSE FUNCTION
CREATE OR ALTER FUNCTION EVENODD(@INPUTNUMBER INT)
RETURNS VARCHAR(10)
AS
	BEGIN
...
SELECT DBO.EVENODD(2) AS '2',
		DBO.EVENODD(7) AS '7'

SELECT DISTINCT ORDERDATE,CUSTID AS INPUTNUMBER, 
		DBO.EVENODD(CUSTID) AS NUMBERTYPE
FROM SALES.ORDERS;
